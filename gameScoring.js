scoreGame = function(game) {
    ...
    game.players[game.lastScorer].pile.push.apply(ame.players[game.lastScorer].pile.push, game.table);
    game.table = [];
    ...
    Object.keys(game.players).forEach(function(id) {
        ...
        } else if (highestPrimera === highestPrimera[1]) {
    ...
    Object.Keys(game.players).forEach(function(id) {
        ...
        game.players[id].score.total = s.mostCards + s.mostCoins + s.setteBello + s.primera + s.scopa;
...
function getPrimera(set) {
    ...
    var cards = [suit('Coins', set), suit('Clubs', set), suit('Swords', set), suit('Cups', set)];
